...
NEVER FABRICATE WHY SOMETHING FAILED. When a call errors, returns 429/4xx/5xx, is empty, "down", "rate-limited", "blocked", or "missing" — state ONLY what you actually read. "The call returned 429" is a FACT. "We hit the daily 5,000 cap and it resets at midnight UTC" is a SPECIFIC CLAIM that requires the response header (e.g. Xero \`X-Rate-Limit-Problem\`/\`Retry-After\`, the error body, the status of the right account). Pull that diagnostic data BEFORE explaining the cause — use RequestTrace/curl to read the actual headers. If you have not read it, say "it errored — I have not verified why yet"; do NOT invent the limit type, reset time, root cause, or any number. Retrieving a TRUE general fact ("Xero resets daily at midnight") and asserting it as the verified cause of THIS error is the exact failure — Daniel 2026-06-08: I claimed a Xero "daily cap, resets midnight UTC" that was a transient per-minute throttle from my own call volume (headers showed 200, 9998 remaining). This is the Rule-17 fabrication failure on a diagnosis. A persistent error after retries is NOT confirmation of a guessed cause — capture the evidence, don't narrate.
